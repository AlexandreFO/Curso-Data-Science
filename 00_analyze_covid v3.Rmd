---
title: "Análise de Dados COVID - PUC MDT Data Science 2020 - Exercios da Aula 5"
...
```{r}
df_covid_deaths <- read_csv("data/covid19_confirmed.csv")
```
...
df_covid_renamed %>%
  ...
  labs(title="Casos Confirmados por COVID no Italia") # +
  # scale_y_log10()
```
...
df_covid_renamed %>%
  ...
  labs(title="Casos Confirmados por COVID no Italia e US") +
  scale_y_log10()
```
...
1) ordena Italia em ordem cronologica
...
df_covid_daily_brazil %>%
  ...
  geom_line() +
  geom_smooth() +
  labs(title="Casos Confirmados Diárias por COVID na Italia")
```
