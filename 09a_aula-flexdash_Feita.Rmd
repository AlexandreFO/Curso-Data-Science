...
# Essa sintaxe -  sum(na.rm=T)  - exclui NA
...
furtos_total <- df_crimes %>% filter(grupo=="Furtos") %>% pull(contagem) %>% sum(na.rm=T)
...
### Furtos de `r ano_min` a `r ano_max` (Milhões)

<!-- ícones em: https://fontawesome.com/v4.7.0/icons/ -->

```{r,message=F}
valueBox(round(furtos_total/10^6,1), icon = "fa-credit-card-alt",
         color="green")
```
...
df_crimes %>%
  ...
  summarize(contagem=sum(contagem,na.rm=T)) %>%
  arrange(contagem) %>%
  mutate(Regiao=Regiao %>% fct_inorder() %>% fct_rev()) %>%
...
### Crimes no municipio do Rio de Janeiro
...
### Crimes Por Mês em 2019 no Rio de Janeiro
